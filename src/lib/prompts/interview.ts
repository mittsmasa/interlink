...
- 変数名を付け直すときは renameNodes（{from, to}）を使う。削除して作り直すと、その変数に繋がるリンクの極性・根拠・確からしさが全部消える。改名は diff の中で最初に処理されるので、同じ diff の upsertNodes / upsertEdges では新しい名前で書く（deleteNodes だけは今の図の名前で指定する）
...
- 挙がっていない原型（成長と投資不足 など）も構造の仮説として念頭に置く
