...
補足（実装上の注意）: 上の「単位」のものさしは kind を決めるためのものではなく、**決めた kind が正しいかを後から確かめる**ために使う。コードは `unit` 列で kind を推論しない（昇格は 3 章のとおり対話で決める）。ただし flow → stock の単位が「ストックの単位 / 時間」になっているかは lint が読む（`unit-mismatch-flow`。7 章の表）。単位表記は自由文字列なので、読めない表記は黙って見送る（`src/lib/diagram/units.ts`）。
...
- 単位の整合チェックをどこまで広げるか（flow → stock の率かどうかは `unit-mismatch-flow` で見るようになった。式の中の単位（`残高 * 利率` の次元）や、`simConfig` に時間単位を持たせて flow の分母と突き合わせる話は未着手）
